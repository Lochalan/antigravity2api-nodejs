 ...
 * Thresholds are dynamically calculated from user-configured memoryThreshold (MB)
...
/**
 * Calculate thresholds based on user-configured memory threshold
 * @param {number} thresholdMB - User-configured memory threshold (MB), i.e. high pressure threshold
 * @returns {Object} Threshold levels (bytes)
 */
...
// Object pool max size config (adjusted by pressure)
const POOL_SIZES = {
...
class MemoryManager {
  ...
  /**
   * Get object pool size config for current pressure
   */
  getPoolSizes() {
  ...
  /**
   * Check memory and trigger appropriate cleanup
   */
  check() {
    ...
    // Execute strategy based on pressure level
    switch (newPressure) {
...
// Unified wrapper for registering cleanup callbacks, maintaining consistent style across modules
