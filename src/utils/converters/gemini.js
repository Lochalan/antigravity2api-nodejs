...
function processModelThoughts(content, reasoningSignature, reasoningContent, toolSignature, toolContent, enableThinking) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if ((!part.thoughtSignature) && (part.functionCall || part.inlineData)) {
      ...
      // Use reasoningSignature first since that's what gets cached from responses
      const partFallback = reasoningSignature || toolSignature || fallbackSig;
...
export function generateGeminiRequestBody(geminiBody, modelName, token) {
  ...
  if (request.contents && Array.isArray(request.contents)) {
    ...
    // Use original modelName for cache lookup (client.js caches with requestBody.model, not mapped name)
    const { reasoningSignature, reasoningContent, toolSignature, toolContent } = getSignatureContext(token.sessionId, modelName, hasTools);
