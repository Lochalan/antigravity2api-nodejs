...
// Cache TTL dynamically adjusted based on memory pressure
...
if (config.useNativeAxios === true) {
  ...
  try {
    ...
    console.warn('AntigravityRequester init failed, falling back to axios:', error.message);
    useAxios = true;
  }
}
...
// Register cleanup callbacks on init
...
async function handleApiError(error, token) {
  ...
  if (status === 403) {
    ...
    throw createApiError(`This account has no permission, auto-disabled. Error details: ${errorBody}`, status, errorBody);
  }
...
export async function generateAssistantResponse(requestBody, token, callback) {
  ...
  // Temporarily cache thinking signature in state for multi-chunk reuse, with session and model info for global cache
  const state = {
    toolCalls: [],
  ...
  try {
    if (useAxios) {
      ...
      // Use Buffer directly to avoid toString memory allocation
    ...
    releaseLineBuffer(lineBuffer); // Ensure return to pool
    await handleApiError(error, token);
  }
}

// Internal helper: fetch raw model data from remote
...
export async function getAvailableModels() {
  ...
  if (!data) {
    // fetchRawModels handles errors, fallback to default list
    return getDefaultModelList();
  }
  ...
  // Add default models if not in API response
...
export async function generateAssistantResponseNoStream(requestBody, token) {
  ...
  for (const part of parts) {
    if (part.thought === true) {
      // Thinking content - use DeepSeek format reasoning_content
