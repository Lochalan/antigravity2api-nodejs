/**
 * SSE streaming response and heartbeat mechanism module
 * Provides unified streaming response handling, keepalive, 429 retry, etc.
 */
...
// ==================== Precompiled constant strings (avoid repeated creation) ====================
...
export const with429Retry = async (fn, maxRetries, loggerPrefix = '') => {
  ...
  let attempt = 0;
  // First execution + up to retries attempts
  while (true) {
    try {
