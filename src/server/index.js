/**
 * Server main entry
 * Express app config, middleware, route mounting, server start and shutdown
 */
...
logger.info(`Static files directory: ${getRelativePath(publicDir)}`);
  ...
  if (req.path.startsWith('/v1/')) {
    ...
    if (apiKey) {
      ...
      if (providedKey !== apiKey) {
        logger.warn(`API Key verification failed: ${req.method} ${req.path} (provided key: ${providedKey ? providedKey.substring(0, 10) + '...' : 'none'})`);
